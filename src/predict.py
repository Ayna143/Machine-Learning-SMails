...
class SpamDetector:
    ...
    def predict_batch(self, texts, model_name=None):
        """
        Classify many emails efficiently: one TF-IDF transform + one model call
        instead of repeating that work per row (which was the main slowdown).
        """
        ...
        tfidf = self.vectorizer.transform(cleaned)
        feat_matrix = extract_features_batch(texts)
        combined = hstack([tfidf, csr_matrix(feat_matrix)])
        ...
        for i, raw in enumerate(texts):
            row_feats = {FEATURE_NAMES[j]: float(feat_matrix[i, j]) for j in range(len(FEATURE_NAMES))}
            pred = int(preds[i])
            conf = float(np.max(probas[i])) if probas is not None else None
            out.append({
                'prediction': 'spam' if pred == 1 else 'not spam',
                'is_spam': bool(pred == 1),
                'confidence': conf,
                'features': row_feats,
                'reasons': self._build_reasons(raw, row_feats, '', ''),
                'model_used': model_name,
            })
        return out
